...
namespace odm.ui.viewModels {
    public class DigitalIOViewModel:ViewModelDeviceBase {
        public DigitalIOViewModel(IUnityContainer container):base(container) {
            ...
            RelayModes = new List<RelayMode>();
            RelayModes.Add(RelayMode.Bistable);
            RelayModes.Add(RelayMode.Monostable);

            IdleStates = new List<RelayIdleState>();
            IdleStates.Add(RelayIdleState.closed);
            IdleStates.Add(RelayIdleState.open);
        }
        ...
        public List<RelayIdleState> IdleStates { get; set; }
        public List<RelayMode> RelayModes { get; set; }

        void SetDelay() {
            if (SelectedRelay == null)
                return;
            try {
                Delay = System.Xml.XmlConvert.ToTimeSpan(SelectedRelay.Properties.DelayTime).Seconds.ToString();
            } catch (Exception err) {
        ...
        string GetDelay() {
            ...
            var ts = TimeSpan.FromSeconds(idelay);
            string val = System.Xml.XmlConvert.ToString(ts);
            return val;
