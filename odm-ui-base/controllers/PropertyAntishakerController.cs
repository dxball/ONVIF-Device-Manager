...
namespace odm.controllers {
	public class PropertyAntishakerController : BasePropertyController {	
		...
		protected override void LoadControl() {
			_devModel = new AntishakerModel(CurrentChannel.profileToken);
			_subscription = _devModel.Load(CurrentSession)
				.Subscribe(arg => {
					...
					UIProvider.Instance.GetAntishakerProvider().InitView(_devModel, dprocinfo, ApplyChanges, CancelChanges);
				}, err => {
					OnCriticalError(err);
				});
		}
