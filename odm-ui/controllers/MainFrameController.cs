...
namespace nvc.controllers {
	public class MainFrameController : BasePropertyController {
		...
		MainFrameProvider _mainFrameProvider = UIProvider.Instance.MainFrameProvider;
			...
			switch (_devModel.devInfo.Manufacturer) {
				case "Synesis":
					if (_devModel.devInfo.Model.Contains("DK-6467-CAM")) {
						_deviceImage = nvc.properties.Resources.mc;
					}
					if (_devModel.devInfo.Model.Contains("DK-6467-ENC2")) {
						_deviceImage = nvc.properties.Resources.mb;
					}
					break;
				case "AXIS":
					if (_devModel.devInfo.Model.Contains("AXIS P3301")) {
						_deviceImage = nvc.properties.Resources.axis_p_3301;
					}
					break;
				default:
					_deviceImage = nvc.properties.Resources.other;
					break;
			}
		}
		...
		public void StopVideoStreaming(ChannelDescription chan) {
		...
		public void StartVideoStreaming(ChannelDescription chan) {
			...
			try {
				...
				dataProcInfo.VideoProcess.StartInfo.UseShellExecute = false;

				dataProcInfo.VideoProcess.StartInfo.FileName = "odm-player-host.exe";
				dataProcInfo.VideoProcess.StartInfo.CreateNoWindow = true;
		...
		void Start(DataProcessInfo dataProcInfo) {
			...
			LogUtils.WriteInfo("dataProcInfo.VideoProcess.Start");
			...
			LogUtils.WriteInfo("dataProcInfo.iPlayer = DuplexChannelFactory<IPlayer>.CreateChannel");
			...
			LogUtils.WriteInfo("dataProcInfo.iPlayer.SetVideoBuffer");
			dataProcInfo.iPlayer.Play(dataProcInfo.Channel.mediaUri);
			LogUtils.WriteInfo("dataProcInfo.iPlayer.Play");
			dataProcInfo.iPlayer.Subscribe();
			LogUtils.WriteInfo("dataProcInfo.iPlayer.Subscribe");
		}

		public void ReloadModel() {
			ReleaseContainer();
			UIProvider.Instance.MainFrameProvider.ClearPropertyContainer();
			UIProvider.Instance.MainFrameProvider.ClearUI();
		...
		protected override void LoadControl() {
			...
			_subscription = _devModel.Load(CurrentSession).Subscribe(arg => {
				...
				_mainFrameProvider.InitView(_devModel, ChannelSelected, _deviceImage);
		...
		void OpenLastOrDefaultControl(){
			...
			if (devState.Channel == null) {
				...
				BasePropertyController propCtrl = linkBtn.CreatePropertyAction();
				UIProvider.Instance.MainFrameProvider.SetLinkSelection();
				propCtrl.CreateController(linkBtn.ModelSession, linkBtn.Channel);
			}
		}
		void OnLoadingError(Exception err) {
			UIProvider.Instance.InfoFormProvider.DisplayLoadingDeviceErrorForm(err, ReturnToDeviceList);
		}
			...
			_buttonsList.ForEach(x => {
				if (x.Channel == chan) {
					if (x.LinkChannelID == LinkButtonsChannelID.Annotation)
						x.IsChecked = chan.modules.DisplayAnnotation;
					if (x.LinkChannelID == LinkButtonsChannelID.Tracker)
						x.IsChecked = chan.modules.ObjectTracker;
				}
			});
		}
		void CreateChannelControlLinkButtons(ChannelDescription channel) {
			...
			//if (_devModel.capabilities. == null)
			//    link.Enabled = false;
			//else
			//    link.Enabled = _devModel.capabilities.Events.WSPullPointSupport;
			link.Enabled = true;
			...
			link.Click = LbtnClick;
			if (_devModel.capabilities.Analytics == null)
				link.Enabled = false;
			else
				link.Enabled = _devModel.capabilities.Analytics.AnalyticsModuleSupport;
			...
			link.IsChecked = channel.modules.ObjectTracker;
			if (_devModel.capabilities.Analytics == null)
				link.Enabled = false;
			else
				link.Enabled = _devModel.capabilities.Analytics.AnalyticsModuleSupport;
			...
			if (_devModel.capabilities.Analytics == null)
				...
				link.Enabled = _devModel.capabilities.Analytics.RuleSupport;
			//Disable Rule engine
			link.Enabled = false;
			link.Click = LbtnClick;
			...
			link.CheckBoxSwitchedAcion = CheckBoxSwitched;
			link.IsCheckable = true;
			link.IsChecked = channel.modules.DigitalAntishaker;
			if (_devModel.capabilities.Analytics == null)
				link.Enabled = false;
			else
				link.Enabled = _devModel.capabilities.Analytics.AnalyticsModuleSupport;
		...
		//    UIProvider.Instance.MainFrameProvider.ReleaseAll();//.DisposeChilds();
		//}
		...
		void ChBOnApply(string message) {
			...
			UIProvider.Instance.InfoFormProvider.DisplayOnCheckBoxApplyChangesForm(message);
		}
		...
		void SnapShotSubscription(Session currenSession, string SourceToken) {
			_eventsImageSubscription = currenSession.GetSnapshot(NvcHelper.GetChannelProfileToken(SourceToken)).ObserveOn(SynchronizationContext.Current)
				.Subscribe(img => {
					...
					UIProvider.Instance.MainFrameProvider.SetChannelImage(CurrentImage, SourceToken);
	...
	public class PlayerCallbacks : IPlayerCallbacks {
		...
		public void LogMessageAcquired(LogMessage logMessage) {
			//LogUtils.WriteEvent();
			DebugHelper.Error("LogMessageAcquired");
		}
	}
