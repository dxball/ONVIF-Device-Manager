...
namespace nvc.controllers {
	public class PropertyDepthCalibrationController : BasePropertyController {
		...
		protected override void LoadControl() {
			...
			_subscription = _devModel.Load(CurrentSession)
				.Subscribe(arg => {
					...
					UIProvider.Instance.DepthCalibrationProvider.InitView(_devModel, dprocinfo, ApplyChanges, CancelChanges);
				}, err => {
					OnCriticalError(err);
				});
		}
		...
		protected override void ApplyChanges() {
			UIProvider.Instance.ReleaseDepthCalibrationProvider();
