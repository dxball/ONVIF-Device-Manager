	...
	public class PropertyMaintenanceController : BasePropertyController {
		...
		protected override void LoadControl() {
			...
			_subscription = _devModel.Load(CurrentSession).Subscribe(arg => {
				UIProvider.Instance.MaintenanceProvider.InitView(_devModel, UpgradeFirmware, SoftReset);
			}, err => {
				OnCriticalError(err);
			});
		}
		...
		void UpgradeCompleate() {
			...
			UIProvider.Instance.InfoFormProvider.DisplayInformationForm(PropertyMaintenanceStrings.Instance.updateCompleate, Close);			
		}
