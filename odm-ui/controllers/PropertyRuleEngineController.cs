...
namespace nvc.controllers {
	public class PropertyRuleEngineController : BasePropertyController {
		...
		protected override void LoadControl() {
			...
			_subscription = _devModel.Load(CurrentSession)
				.Subscribe(arg => {
					...
					UIProvider.Instance.RuleEngineProvider.InitView(_devModel, dprocinfo, ApplyChanges, CancelChanges);
				}, err => {
					OnCriticalError(err);
				});
		}
		...
		void AddRule() {
			RuleDescriptor descr = new RuleDescriptor() { name = "Rule", enabled = true };
