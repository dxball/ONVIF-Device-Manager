...
namespace nvc.controls
{
    public partial class PropertyEvents : BasePropertyControl{
		...
		void _lviewEvents_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e) {
			if (e.Item != null)
				if (e.Item.Tag != null) {
					...
					try {
						_imgBox.CreateBinding(x => x.Image, (nvc.controllers.EventDescriptor)e.Item.Tag, x => x.Screen);
					} catch (Exception err) {
        ...
        public void FillListView(List<nvc.controllers.EventDescriptor> lstEvents)
        {
			_lviewEvents.Items.Clear();
			lstEvents.ForEach(x => { AddListItem(x); });
        }
		public void RemoveListViewItem(nvc.controllers.EventDescriptor evDescr) {
			_lviewEvents.Items.ForEach(itm => { if (((ListViewItemBindable)itm).Tag == evDescr) ((ListViewItemBindable)itm).Remove(); });
		}
		public void AddListItem(nvc.controllers.EventDescriptor evDescr)
        {
