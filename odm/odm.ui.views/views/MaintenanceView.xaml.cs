		...
		private void Init(Model model) {
			...
			BackupCommand = new DelegateCommand(
				() => {
					...
					dlg.FileName = maintenanceLinkEventArgs.Manufacturer + "-" + maintenanceLinkEventArgs.DeviceModel + "-" + model.firmwareVersion + "-backup";
					//dlg.InitialDirectory = Directory.GetCurrentDirectory();
					if (dlg.ShowDialog() == true) {
						Success(new Result.Backup(dlg.FileName));
					}
				},
				() => model != null && model.capabilities.Device.System.SystemBackup
			);
