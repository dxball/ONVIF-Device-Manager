...
namespace nvc.models {
	public static class NetworkSettingsExtensions {
		public static IObservable<NetworkSettings> GetNetworkSettings(this Session session) {

			var proxy = session.device;

			return Observable.Join(proxy.GetNetworkDefaultGateway()
				.And(proxy.GetDNS())
				.And(proxy.GetNetworkInterfaces())
				.Then((gateway, dns, nics) => {
					var networkSettngs = new NetworkSettings();
					
					if (gateway.IPv4Address != null && gateway.IPv4Address.Count() > 0) {
						networkSettngs.defaultGateway = IPAddress.Parse(gateway.IPv4Address[0]);
					}

					if (dns.DNSManual != null && dns.DNSManual.Count() > 0) {
						networkSettngs.staticDns = IPAddress.Parse(dns.DNSManual[0].IPv4Address);
					} else if (dns.DNSFromDHCP != null && dns.DNSFromDHCP.Count() > 0) {
						networkSettngs.staticDns = IPAddress.Parse(dns.DNSFromDHCP[0].IPv4Address);
					}
					...
					if (nic != null) {
						...
						networkSettngs.dhcp = nic.IPv4.Config.DHCP;

						if (nic_cfg.Manual.Count() > 0) {
							networkSettngs.staticIp = IPAddress.Parse(nic_cfg.Manual[0].Address);
							networkSettngs.subnetPrefix = nic_cfg.Manual[0].PrefixLength;
						} else if (nic_cfg.FromDHCP != null) {
							networkSettngs.staticIp = IPAddress.Parse(nic_cfg.FromDHCP.Address);
							networkSettngs.subnetPrefix = nic_cfg.FromDHCP.PrefixLength;
						}
					}

					return networkSettngs;
				})
			);			
		}
		...
		public static IObservable<DeviceDescription> Save(this NetworkSettings netSettings, Session session) {
			...
			var proxy = session.device;
			var id = session.deviceDescription.epMetadata.Address.Uri;
